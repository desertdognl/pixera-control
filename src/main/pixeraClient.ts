...
export class PixeraClient {
  ...
  async connect(host: string, port: number, timeoutMs = 5000): Promise<number> {
    const hostTrimmed = host.trim()
    this.host = hostTrimmed
    this.port = port
    log(`connecting ${hostTrimmed}:${port} framing=pxr1`)
    await this.openSocket(hostTrimmed, port, timeoutMs)
    log(`tcp open ${hostTrimmed}:${port}`)
  ...
  private async readApiRevision(): Promise<number | null> {
    try {
      ...
      log(`api revision failed: ${error instanceof Error ? error.message : String(error)}`)
      return null
    }
  }
  ...
  async call<T = unknown>(method: string, params?: Record<string, unknown>, timeoutMs = 4000): Promise<T> {
    ...
    const packet = this.framing === 'delimiter' ? Buffer.from(json + DELIMITER, 'utf8') : encodePxr1(json)

    const result = await new Promise<unknown>((resolve, reject) => {
      const timer = setTimeout(() => {
        this.pending.delete(id)
        const error = new Error(`Pixera did not answer ${method}`)
        log(error.message)
        reject(error)
      }, timeoutMs)
  ...
  async close(): Promise<void> {
    ...
    this.buffer = Buffer.alloc(0)
    if (!socket) return
    log(`closing ${this.host}:${this.port}`)
  ...
  private onClose(): void {
    ...
    log(`disconnected ${this.host}:${this.port}`)
    this.connected = false
    this.socket = null
  ...
  private dispatch(raw: string): void {
    ...
    if (message.result && typeof message.result === 'object' && '_pixc' in (message.result as object)) {
